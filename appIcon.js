...
export class BaseButton extends St.Widget {
    ...
    constructor() {
        ...
        this.connect('button-press-event', (_actor, event) => this._handleButtonPress(event));
        this.connect('button-release-event', (_actor, event) => this._handleButtonRelease(event));
        this.connect('touch-event', (_actor, event) => this._handleTouchEvent(event));
    ...
    _cancelLongPressTimeout() {
        this._longPressTimeoutId = Utils.safeSourceRemove(this._longPressTimeoutId);
    }

    _beginPress(button, isTouch = false) {
        if (this._isDestroyed)
            return;

        this._pressedButton = button;
        this._pressedTouch = isTouch;
        this._longPressTriggered = false;
        this._onPressed({pressed: true});
        ...
        this._longPressTimeoutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT, LONG_PRESS_TIMEOUT_MS, () => {
            this._longPressTimeoutId = 0;

            if (this._isDestroyed || this._dragging || this._pressedButton === null)
                return;
    ...
    _handleButtonPress(event) {
        ...
        this._beginPress(event.get_button());
        return Clutter.EVENT_PROPAGATE;
    }

    _handleButtonRelease(event) {
        if (this._isDestroyed || this._pressedTouch || this._pressedButton !== event.get_button())
            return Clutter.EVENT_PROPAGATE;

        const longPressTriggered = this._longPressTriggered;
        this._endPress();

        if (this._dragging || longPressTriggered)
            return Clutter.EVENT_PROPAGATE;
    ...
    _handleTouchEvent(event) {
        ...
        case Clutter.EventType.TOUCH_BEGIN:
            this._beginPress(0, true);
            return Clutter.EVENT_PROPAGATE;
        case Clutter.EventType.TOUCH_END: {
            if (!this._pressedTouch)
                return Clutter.EVENT_PROPAGATE;
    ...
    showLabel() {
        if (this._isDestroyed || !this.tooltipLabel)
            return;
    ...
    _onDestroy() {
        ...
        this.tooltipLabel?.remove_all_transitions();
        this.tooltipLabel?.hide();
        Utils.safeDestroy(this.tooltipLabel);
    ...
    _animateAppIcon(isMinimized) {
        if (this._isDestroyed || !St.Settings.get().enable_animations)
            return;
...
export class AppIcon extends BaseButton {
    ...
    _removeAnimateUrgentId() {
        this._animateUrgentId = Utils.safeSourceRemove(this._animateUrgentId);
    }
    ...
    _onLongPress(event) {
        const isPrimaryOrTouch = event.get_button() === Clutter.BUTTON_PRIMARY || event.get_button() === 0;
        if (isPrimaryOrTouch) {
            this.popupMenu();
            return true;
        }
    ...
    _onMouseScroll(actor, event) {
        ...
        if (scrollAction === ScrollAction.CYCLE && direction) {
            if (!this._scrollTimeOutId) {
                this._scrollTimeOutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT, 300, () => {
    ...
    _onDestroy() {
        if (this._appIconDestroyed)
            return;

        this._appIconDestroyed = true;
        global.display.disconnectObject(this);
        this.app?.disconnectObject(this);
        this._previewMenu?.disconnectObject(this);
        this._menu?.disconnectObject?.(this);
        Utils.safeDestroy(this.notificationBadges);
        this.notificationBadges = null;
        ...
        if (this._menu) {
            this._menu.close();
            Utils.safeDestroy(this._menu);
            this._menu = null;
        }

        this._scrollTimeOutId = Utils.safeSourceRemove(this._scrollTimeOutId);

        this._previewMenu?.close();
        Utils.safeDestroy(this._previewMenu);

        this._dragBeginId = Utils.safeDisconnect(this._draggable, this._dragBeginId);
        this._dragCancelledId = Utils.safeDisconnect(this._draggable, this._dragCancelledId);
        this._dragEndId = Utils.safeDisconnect(this._draggable, this._dragEndId);
    ...
    _onDragCancelled() {
        Utils.removeChildFromParent(this.mainBox, this);
    ...
    _removePreviewMenuTimeout() {
        this._previewMenuTimeoutId = Utils.safeSourceRemove(this._previewMenuTimeoutId);
    }
    ...
    _setPreviewPopupTimeout() {
        ...
        this._removePreviewMenuTimeout();
        this._previewMenuTimeoutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT,
            delay, () => {
                if (this._isDestroyed) {
    ...
    _disconnectWindowMinimizeEvent() {
        ...
        windows.forEach(window => {
            if (window._windowMinimizeId > 0) {
                window._windowMinimizeId = Utils.safeDisconnect(window, window._windowMinimizeId);
            }
        });
    }
    ...
    _removeCylceWindowsTimeout() {
        this._cylceWindowsTimeoutId = Utils.safeSourceRemove(this._cylceWindowsTimeoutId);
    }
    ...
    _setCylceWindowsTimeout() {
        ...
        this._cylceWindowsTimeoutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT, 2000, () => {
            if (this._isDestroyed) {
