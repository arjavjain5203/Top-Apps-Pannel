...
class topAppsProAppIconsTaskbar extends St.ScrollView {
    ...
    _redisplay() {
        ...
        this.mainBox.get_children().forEach(actor => {
            if (actor instanceof AppIcon) {
                ...
                Utils.removeChildFromParent(this.mainBox, actor);
            } else {
                Utils.removeChildFromParent(this.mainBox, actor);
                Utils.safeDestroy(actor);
            }
        });
    ...
    _disconnectWorkspaceSignals() {
        if (this._lastWorkspace) {
            this._workspaceWindowAddedId = Utils.safeDisconnect(this._lastWorkspace, this._workspaceWindowAddedId);
            this._workspaceWindowRemovedId = Utils.safeDisconnect(this._lastWorkspace, this._workspaceWindowRemovedId);
    ...
    removeWindowPreviewCloseTimeout() {
        this._windowPreviewCloseTimeoutId = Utils.safeSourceRemove(this._windowPreviewCloseTimeoutId);
    }
    ...
    setWindowPreviewCloseTimeout() {
        ...
        this._windowPreviewCloseTimeoutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT,
    ...
    _destroy() {
        ...
        this._clearConnections();
        this.showAppsIcon = Utils.safeDestroy(this.showAppsIcon);
        [...this.appIconsCache.entries()].forEach(([appID, appIcon]) => {
            appIcon.stopAllAnimations();
            Utils.safeDestroy(appIcon);
            this.appIconsCache.delete(appID);
        });
