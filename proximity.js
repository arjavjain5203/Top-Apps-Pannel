...
export class ProximityWatch {
    ...
    destroy() {
        this._allocationChangedId = Utils.safeDisconnect(this.actor, this._allocationChangedId);
    }
...
export const ProximityManager = class {
    ...
    destroy() {
        ...
        this._queueUpdateId = Utils.safeSourceRemove(this._queueUpdateId);
    ...
    _disconnectFocusedWindow(destroy) {
        if (this._focusedWindowInfo && !destroy) {
            this._focusedWindowInfo.allocationId = Utils.safeDisconnect(
                this._focusedWindowInfo.window, this._focusedWindowInfo.allocationId);
            this._focusedWindowInfo.destroyId = Utils.safeDisconnect(
                this._focusedWindowInfo.window, this._focusedWindowInfo.destroyId);
        }
    ...
    _queueUpdate(noDelay) {
        ...
        this._queueUpdateId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT, MIN_UPDATE_MS, () => {
            if (this._isDestroyed) {
