...
export const Intellihide = class {
    ...
    destroy() {
        ...
        this._intellihideChangedId = Utils.safeDisconnect(TaskbarManager.settings, this._intellihideChangedId);
        this._intellihideOnlySecondaryChangedId = Utils.safeDisconnect(TaskbarManager.settings, this._intellihideOnlySecondaryChangedId);
    ...
    _setTrackPanel(enable) {
        if (this._isDestroyed || !this._panelBox)
            return;
    ...
    _removeRevealMechanism() {
        if (this._pointerWatch?.remove)
            this._pointerWatch.remove();
        else if (this._pointerWatch)
    ...
    _removeTimeout(idName) {
        this[idName] = Utils.safeSourceRemove(this[idName]);
    }

    _createTimeout(idName, delay, callback) {
        this._removeTimeout(idName);
        this[idName] = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT, delay, () => {
            this[idName] = null;
            if (this._isDestroyed)
                return;
