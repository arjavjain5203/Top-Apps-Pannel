...
export const PanelManager = class topAppsProPanelManager {
    ...
    _updateOverviewPanelStyle() {
        if (this._isDestroyed || !this._panel)
            return;
    ...
    _destroyWeatherWidget() {
        this._updateWeatherTimeoutId = Utils.safeSourceRemove(this._updateWeatherTimeoutId);
        ...
        Utils.removeChildFromParent(this._customDateMenuBox, this._clockDisplay);
        this._clockDisplay.set_style_class_name('clock');
        if (this._origDateMenuBox && this._clockDisplay.get_parent() !== this._origDateMenuBox)
            this._origDateMenuBox.insert_child_at_index(this._clockDisplay, 1);

        this._weatherClient?.disconnectObject(this);
        this._customDateMenuBox = Utils.safeDestroy(this._customDateMenuBox);
        this._weatherBox = null;
    ...
    _moveWeatherWidget(weatherPosition) {
        if (this._isDestroyed || !this._customDateMenuBox)
            return;

        Utils.removeChildFromParent(this._customDateMenuBox, this._weatherBox);
        Utils.removeChildFromParent(this._customDateMenuBox, this._clockDisplay);
    ...
    _createWeatherWidget(weatherPosition) {
        if (this._isDestroyed || this._hasWeatherWidget)
            return;
        ...
        Utils.removeChildFromParent(this._origDateMenuBox, this._clockDisplay);
        ...
        this._updateWeatherTimeoutId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 300, () => {
            if (this._isDestroyed || !this._weatherClient)
    ...
    _syncWeather() {
        if (this._isDestroyed || !this._weatherClient || !this._weatherBox)
            return;
    ...
    _setElementPosition(element, position, offset) {
        ...
        if (parent)
            Utils.removeChildFromParent(parent, element);
    ...
    setSizeAndPosition() {
        ...
        if (panelLocation === PanelLocation.TOP) {
            ...
            if (this._panelHeightSignal) {
                this._panelHeightSignal = Utils.safeDisconnect(this._panelBox, this._panelHeightSignal);
            }
    ...
    destroy() {
        ...
        if (!this.isMainPanel) {
            ...
            Utils.safeDestroy(this.panelBox);
        } else {
            this._clockDisplay.text = this._clock.clock;
            ...
            Utils.safeDestroy(this._appIconsTaskbar);
        }
