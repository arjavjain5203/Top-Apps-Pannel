...
export const WindowPreviewMenuManager = class topAppsProWindowPreviewMenuManager extends PopupMenu.PopupMenuManager {
    constructor(owner, grabParams) {
        ...
        this._owner.connect('destroy', () => {
            ...
            this._keyFocusId = Utils.safeDisconnect(global.stage, this._keyFocusId);
    ...
    _removeChangeWindowPreviewTimeout() {
        this._changeWindowPreviewTimeoutId = Utils.safeSourceRemove(this._changeWindowPreviewTimeoutId);
    }
};
...
export const WindowPreviewMenu = class topAppsProWindowPreviewMenu extends PopupMenu.PopupMenu {
    ...
    display() {
        ...
        this._previewBox = Utils.safeDestroy(this._previewBox);
    ...
    _onDestroy() {
        ...
        if (this._source)
            this._mappedId = Utils.safeDisconnect(this._source, this._mappedId);
...
class topAppsProWindowPreviewMenuItem extends PopupMenu.PopupBaseMenuItem {
    ...
    _cloneTexture(metaWin) {
        if (this._isDestroyed || !metaWin)
            return;

        if (!this._width || !this._height) {
            this._cloneTextureLater = Utils.laterAdd(Meta.LaterType.BEFORE_REDRAW, () => {
                if (this._isDestroyed || !this._window) {
                    this._cloneTextureLater = 0;
        ...
        this._clone.connect('destroy', () => {
            if (this._destroyId) {
                this._destroyId = Utils.safeDisconnect(mutterWindow, this._destroyId);
            }
            this._clone = null;
        });
    }
    ...
    _onWindowAdded(workspace, win) {
        ...
        if (win.get_transient_for() === metaWindow) {
            this._windowAddedId = Utils.safeDisconnect(workspace, this._windowAddedId);
    ...
    _startPeek() {
        ...
        if (this._source.peekTimeoutId > 0) {
            this._source.peekTimeoutId = Utils.safeSourceRemove(this._source.peekTimeoutId);
        }

        if (this._source.peekInitialWorkspaceIndex < 0) {
            this._source.peekTimeoutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT,
    ...
    _peek() {
        this.workspaceSwitchId = Utils.safeSourceRemove(this.workspaceSwitchId);
    ...
    _removeEndPeekDelay() {
        if (this._source)
            this._source.endPeekTimeoutId = Utils.safeSourceRemove(this._source.endPeekTimeoutId);
    }

    _endPeekWithDelay() {
        this._removeEndPeekDelay();
        this._source.endPeekTimeoutId = Utils.timeoutAddOnce(GLib.PRIORITY_DEFAULT, 250, () => {
            if (this._isDestroyed || !this._source) {
                if (this._source)
                    this._source.endPeekTimeoutId = 0;
                return;
            }
    ...
    _onDestroy() {
        ...
        this._isDestroyed = true;
        this._removeEndPeekDelay();
        this._section?.disconnectObject(this);

        if (this._cloneTextureLater) {
            this._cloneTextureLater = Utils.laterRemove(this._cloneTextureLater);
        }

        if (this._windowAddedLater) {
            this._windowAddedLater = Utils.laterRemove(this._windowAddedLater);
        }

        if (this._windowAddedId > 0) {
            this._windowAddedId = Utils.safeDisconnect(this._workspace, this._windowAddedId);
        }

        if (this._destroyId > 0) {
            this._destroyId = Utils.safeDisconnect(this._mutterWindow, this._destroyId);
        }

        if (this._windowTitleId > 0) {
            this._windowTitleId = Utils.safeDisconnect(this._window, this._windowTitleId);
        }
